...
def Gillespie_CIR_2D_p(args):
    ...
    idx = np.round(np.linspace(0, len(R)-1, nT)).astype(int)
    SDE = R[idx]
    return X[-1], SDE
...
if __name__ == "__main__":   
    ...
    parser.add_argument("-n", "--nCell",  default=100, type=int, action='store', help='nCell')
    ...
    for i in range(1,7):
        ...
        mdict={'runtime': trun, 'Ncell': args.nCell, 'Tmax': te, 'dt': dt, 'T_': T_, 'tvec': tvec,
